...
Сервер PostgreSQL создался в контейнере, это как бы отдельный компьютер, имеющий свой собственный IP-адрес (псевдоним localhost). Для связи с этим сервером служат клиентские программы типа pgAdmin или DBeaver, но я предпочитаю пользоваться утилитой psql.
```
